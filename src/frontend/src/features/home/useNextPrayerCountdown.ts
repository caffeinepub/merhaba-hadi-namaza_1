...
export function useNextPrayerCountdown(
  ...
  const lastSentTimestampRef = useRef<{
    name: string;
    timestamp: number;
  } | null>(null);
  const lastSentTimeStringRef = useRef<{ name: string; time: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      ...
      if (foundNext) {
        ...
        if (
          !lastSent ||
          lastSent.name !== foundNext.name ||
          lastSent.timestamp !== nextTimestamp
        ) {
          sendNextPrayerToAndroid(foundNext.name, nextTimestamp);
          lastSentTimestampRef.current = {
            name: foundNext.name,
        ...
        if (
          !lastSentTimeString ||
          lastSentTimeString.name !== foundNext.name ||
          lastSentTimeString.time !== foundNext.time
        ) {
          sendNextPrayerWithTimeString(foundNext.name, foundNext.time);
          lastSentTimeStringRef.current = {
            name: foundNext.name,
            time: foundNext.time,
          };
        }
